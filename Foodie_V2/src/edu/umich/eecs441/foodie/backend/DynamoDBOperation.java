...
public class DynamoDBOperation {
	...
	// TODO: From the perspective of design, the function here should direct from BackendMealEntry Class. Change later
	 ...
	 */
	public void markMeal (BackendMealEntry entry) {
		Map<String, AttributeValue> item = new HashMap<String, AttributeValue>();
		
		item.put("USER_ID", new AttributeValue().withN(String.valueOf(entry.getClientId())));
		item.put("MEAL_NAME", new AttributeValue().withS(entry.getMealName()));
		item.put("MEAL_TRANSLATION", new AttributeValue().withS(entry.getMealTranslation()));
		item.put("PIC_URL", new AttributeValue().withS(entry.getPicUrl()));
